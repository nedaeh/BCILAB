...
predmat = glmnetPredict(glmnet_object, x, options.lambda, type);
...
for i=1:nfolds
    ...
    predmat(which,:) = glmnetPredict(cvfit,x(which,:),options.lambda, type);
end
...
%if verbose, cvglmnetPlot(CVerr.glmnet_object);end
if verbose, glmnetPlot(CVerr.glmnet_object);end
end